...
impl Spine {
    pub fn load(info: SpineDemo) -> Self {
        ...
        let premultiplied_alpha = info.pma;
...
impl EventHandler for Stage {
    ...
    fn draw(&mut self, ctx: &mut Context) {
        let renderables = self.spine.controller.renderables();
        ...
        for (renderable, bindings) in renderables.into_iter().zip(self.bindings.iter_mut()) {
            ...
            for vertex_index in 0..renderable.vertices.len() {
                vertices.push(Vertex {
                    ...
                    color: Color::from(renderable.color),
                    dark_color: Color::from(renderable.dark_color),
                });
            }
